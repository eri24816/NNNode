...
import asyncio
import threading
from objectsync_server import Environment
import json

env_class : Environment.Env = None
envs=dict()
...
async def lobby(websocket : websockets.legacy.server.WebSocketServerProtocol, path):
    print( isinstance(env_class,Environment.Env))
    ...
    async for message in websocket:
        ...
        if command == "stt": # start a env
            ...
            new_env=env_class(name=env_name)
...
def start(env):
    ...
    env_class = env
    if env_class == None:   
        raise Exception("Please assign objectsync_server.server.env_class for intantiating envs")  
    if not issubclass(env_class,Environment.Env):
        raise Exception("objectsync_server.server.env_class should inherit objectsync_server.server.Environment.Env.")  
